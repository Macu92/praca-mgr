...
//
//		CsvColumnWriter csvWriter = new CsvColumnWriter(new File("C:/Users/Maciek/Documents/UCZELNIA/praca-mgr/csv.csv"), new File("C:/Users/Maciek/Documents/UCZELNIA/praca-mgr/csv.work."), 3);
//
//		csvWriter.writeNextCol(Arrays.asList(new String[] { "first0", "first1", "first2", "first3" }));
//		csvWriter.writeNextCol(Arrays.asList(new String[] { "second0", "second1", "second2", "second3" }));
//		csvWriter.writeNextCol(Arrays.asList(new String[] { "third0", "third1", "third2", "third3" }));
//
//	}
	...
	public void writeNextCol(List<String> colOfValues) throws IOException {
		...
		for (String nextColValue : colOfValues) {
			...
			// we must retrieve the line from the duplicated file based on its
			// the ofset and length
		...
		// Though I have not done it myself but obviously some code should be
		// added here to care for the cases where
		// less column values have been provided in this method than the total
		// number of lines
	...
	private int findLineOfset(int lineNo) {
		...
		for (int i = 0; i < lineNo; i++)
			ofset += lineInBytes[lineNo] + (firstColWritten ? 1 : 0); // 1 byte
																		// is
																		// added
																		// for
																		// '\n'
